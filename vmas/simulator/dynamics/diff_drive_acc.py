...
class DiffDriveAcc(Dynamics):
    ...
    def f_vel(self, u_command):
        return self.kp * u_command
    ...
    def process_action(self):
        u_command = self.agent.action.u[:, 0]  # Forward acceleration
        ...
        v_cur_norm = torch.linalg.norm(self.agent.state.vel, dim=-1)

        delta_vel = self.dt * self.f_vel(u_command)

        # Select the integration method to calculate the change in state
        if self.integration == "euler":
            delta_state = self.euler(state, delta_vel + v_cur_norm, ang_vel_command)
        else:
            delta_state = self.runge_kutta(
                state, delta_vel + v_cur_norm, ang_vel_command
            )
