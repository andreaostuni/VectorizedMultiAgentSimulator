...
class SocialForcePolicy(torch.nn.Module):
    ...
    def forward(
        ...
        Returns:
          ...
          agents.shape = (envs, n_agents, n_features)
          (x, y, vx, vy, radius, goal_x, goal_y, group_id)
          social_work (torch.Tensor): Social work acting on the agent
          social_work.shape = (envs, n_agents1)
        """
        (
            forces,
            _,
            ...
            agents_distances,
            obstacle_distances,
        ) = self.compute_forces(agents, box_obstacles, sphere_obstacles, lines)

        self.update_position(agents, forces, self.time_step)

        social_work = self.compute_social_work(
            obstacle_force, social_force, social_work_dual
        )
        return agents, social_work, agents_distances, obstacle_distances
    ...
    def compute_forces(
        ...
        Returns:
          total_forces (torch.Tensor): total forces acting on the agents in the simulation
            total_forces.shape = (envs, n_agents, 2)
          ...
          social_work_dual (torch.Tensor): Sum of the norm of the social forces on the other agents
            social_work_dual.shape = (envs, n_agents)
            agents_distances (torch.Tensor): Distances between the agents
            agents_distances.shape = (envs, n_agents, n_agents)
            obstacle_distances (torch.Tensor): Distances between the agents and the obstacles
            obstacle_distances.shape = (envs, n_agents, n_obstacles)
        """
        ...
        obstacle_force, obstacle_distances = self.compute_obstacle_force(
            agents, box_obstacles, sphere_obstacles, lines
        )  # (envs, n_agents, 2), (envs, n_agents, n_obstacles)

        # compute the social force for each agent
        social_force, social_work_dual, agents_distances = self.compute_social_force(
            agents
        )  # (envs, n_agents, 2), (envs, n_agents), (envs, n_agents, n_agents)
    ...
    def compute_obstacle_force(
        ...
        Returns:
          obstacle_force (torch.Tensor): Obstacle force acting on the agent
          obstacle_force.shape = (envs, n_agents, 2)
            distances (torch.Tensor): Distances between the agents and the obstacles
        ...
        return (
            obstacle_force.sum(dim=2) / (active_obstacles.unsqueeze(-1) + 1e-8),
            distances,
        )
        # (envs, n_agents, 2), (envs, n_agents, n_obstacles)

    def compute_social_force(
        self, agents: torch.Tensor
        ...
        Returns:
          social_force (torch.Tensor): Social force acting on the agent
          social_force.shape = (envs, n_agents, 2)
          social_work_dual (torch.Tensor): Sum of the norm of the social forces caused by the other agents
          social_work_dual.shape = (envs, n_agents)
          distances (torch.Tensor): Distances between the agents
          distances.shape = (envs, n_agents, n_agents)
        """
        ...
        social_force = torch.where(
            agents_distances > self.agent_max_dist,
            torch.zeros_like(social_force),
            social_force,
        )

        social_force_norm = torch.norm(
            social_force, dim=-1
        )  # (n_envs, n_agents, n_agents)

        # subtract from the distance the sum of the radii of the agents
        agents_distances = (
            agents_distances
            - agents[:, :, 4:5].unsqueeze(2)
            - agents[:, :, 4:5].unsqueeze(1)
        )
        # (n_envs, n_agents, n_agents)

        return (
            social_force.sum(dim=2),
            social_force_norm.sum(dim=2),
            agents_distances.squeeze(-1),
        )
        # (n_envs, n_agents, 2) , (n_envs, n_agents), (n_envs, n_agents, n_agents)
    ...
    def compute_social_work(
        self,
        obstacle_force: torch.Tensor,
        social_force: torch.Tensor,
        social_work_dual: torch.Tensor,
    ) -> torch.Tensor:
        """Compute the social work for agents in the simulation.
        ...
        Args:
          ...
          obstacle_force (torch.Tensor): Obstacle force acting on the agent
            obstacle_force.shape = (envs, n_agents, 2)
          social_force (torch.Tensor): Social force acting on the agent
            social_force.shape = (env, n_agents, 2)

        Returns:
          social_work (torch.Tensor): Social work acting on the agent
          social_work.shape = (envs, n_agents, 2)
        """
        # compute the social work for each agent

        # wr is the work of the agent to the other agents and on the environment
        # is the sum of the norm of the social force and the norm of the obstacle force
        wr = torch.norm(social_force, dim=-1) + torch.norm(obstacle_force, dim=-1)

        # wp is the social work provoked by the robot in the other agents
        # is the sum of the norm of the social force
        wp = social_work_dual
