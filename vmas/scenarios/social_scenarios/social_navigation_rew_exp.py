...
from vmas.scenarios.social_scenarios.env_utils import (
    env_utils,
    human_utils,
    reward_utils,
)
...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        self.n_box_obstacles = kwargs.pop("n_box_obstacles", 1)
        ...
        center_objectives = kwargs.pop(
            "center_objectives", [1.0, 0.0, 1.0, 1.0, 2.0, 0.0]
        )
        # Make world

        world = World(batch_dim, device, substeps=2, dt=self.dt)

        self.center_objectives = torch.tensor(
            center_objectives,
            device=device,
        )

        self.sigmas = torch.tensor(
            [0.6, 0.6, 0.8, 0.6, 0.5, 0.6],
            device=device,
        )

        self.lims = torch.tensor(
            [
                [0.0, 2.0],  # Rd: forward movement
                [0.0, 1.0],  # Rh: heading towards the goal
                [0.0, 2.0],  # Rv: linear velocity
                [0.0, 1.0],  # Ro: obstacle avoidance
                [0.0, 2.0],  # Rp: social disturbance
                [0.0, 2.0],  # Rs: social work
            ],
            device=device,
        )
        known_colors = [
    ...
    def reward(self, agent: Agent):
        ...
        with torch.profiler.record_function("reward"):
            ...
            objectives = torch.zeros(self.world.batch_dim, 6, device=self.world.device)
            ...
            if latest_state is not None:
                ...
                self.Rd = forward_movement / (agent.max_speed * self.dt)
                objectives[:, 0] = self.Rd
            ...
            self.Rh = torch.sqrt(torch.abs(angle / torch.pi))
            objectives[:, 1] = self.Rh
            ...
            objectives[:, 3] = self.Ro

            # [penalty] social disturbance
            min_distance_to_agents = (
            ...
            self.Rp = torch.clamp(
                min_distance_to_agents / self.center_objectives[4], min=0.0, max=1.0
            )
            objectives[:, 4] = self.Rp
            self.Rs = 10 * self.human_simulation.social_work[:, agent_index]

            objectives[:, 5] = self.Rs

            if self.scenario_type == human_utils.Scenario.EASY:
                # we consider only the first two objectives
                rew = reward_utils.mog_reward(
                    objectives[:, 0:2],
                    self.center_objectives[0:2],
                    self.sigmas[0:2],
                    self.lims[0:2],
                    A=1.0,
                )
                ...
                rew = reward_utils.mog_reward(
                    objectives, self.center_objectives, self.sigmas, self.lims, A=1.0
                )
    ...
    def info(self, agent: Agent) -> Dict[str, Tensor]:
        ...
        return {
            "episode": {
                "error_distance_mean": (self.center_objectives[0] - self.Rd).mean(),
                "error_heading_mean": (self.center_objectives[1] - self.Rh).mean(),
                "error_velocity_mean": (self.center_objectives[2] - self.Rv).mean(),
                "error_obstacle_mean": (self.center_objectives[3] - self.Ro).mean(),
                "error_proxemics_mean": (self.center_objectives[4] - self.Rp).mean(),
                "error_social_work_mean": (self.center_objectives[5] - self.Rs).mean(),
